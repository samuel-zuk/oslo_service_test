...
def wrap(conf, sock):
    ...
    """
    sock = ssl.wrap_socket(sock, **ssl_kwargs)  # nosec
    return SSLDebugWrapper(sock)
    """
...
class SSLDebugWrapper(object):
    ...
    def write(self, data):
        LOG.debug('SSLSocket outgoing write: %s' % data)
        self.sock.write(data)

    def read(self, size=-1):
        data = self.sock.read(size)
        LOG.debug('SSLSocket incoming read of size %d: %s' % (size, data))
        return data
